...
// }
async function getProductById(id){
    if (await redis.exists(`product:${id}`)) {
      console.log('from cache')
      return JSON.parse(await redis.get(`product:${id}`));
    }
    const product = await productModel.getProductById(id);
  ...
  redis.set(`product:${id}`, JSON.stringify(productDataObject), {
  expiration:{value: 3600, type:"EX"},
  });
...
console.error(`Failed to delete cache for product:${id}`, err);
    }} catch (error) {
  throw error;
}}
